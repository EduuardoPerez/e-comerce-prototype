...
// Configuar el servidor para JSON
app.use(bodyParser.json());
app.use(bodyParser.urlencoded({ extended: true }));

app.use( cors() );

// valida que el web token sea correcto
const jwtCheck = jwt({
      secret: jwks.expressJwtSecret({
          cache: true,
          rateLimit: true,
          jwksRequestsPerMinute: 5,
          jwksUri: 'https://dev-w3hnzwlb.auth0.com/.well-known/jwks.json'
    }),
    audience: 'http://productos',
    issuer: 'https://dev-w3hnzwlb.auth0.com/',
    algorithms: ['RS256']
});
